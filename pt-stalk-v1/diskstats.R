...
ptstalk.diskstats.graphs <- function(sourcedir, targetdir) 
{
	...
	{
		mp <- ggplot(data, aes_string(x="TS",y=j,color="Device")) + geom_point() + theme(axis.text.x = element_text(angle=90,vjust=0.5)) + labs(x = "Time", y = j, title = paste("diskstats - ", j)) 
		#+ scale_y_continuous(breaks=data$ReadsMerged, labels = data$ReadsMerged)
